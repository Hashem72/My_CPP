...
/** this is to find out the TP, FP, TN and FN of features from two narrowpeak files. Note that it is feature-wise and 
not base-wise meaning that if one feature from first file is overlapped with another feature from the second file then 
the tp is counted one
...
void get_tp_tn_fp_fn(std::vector<std::pair<unsigned int, unsigned int>> *first_file_features, 
  ...
  unsigned int  nf1    = first_file_features->size();
  unsigned int  nf2    = second_file_features->size();
  cout<<"Nf1 = "<< nf1 << "  NF2 = "<< nf2<<endl;
  ...
  //in case one different features from first is overlapped with one feature from the second
  //needs a proper way of dealing with that but for time being:
  if(tp > nf2){
    tp = nf2;
  }
  unsigned int fp = nf1 -tp;
  
  unsigned int  fn  = nf2-tp;
